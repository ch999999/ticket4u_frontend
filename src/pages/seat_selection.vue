...
function getMovieName(showInformation){
  movieName.value = showInformation.title
}

function getMovieDate(showInformation){
  movieDate.value = dateFormatter.formatToReadable(showInformation.start_time)
}
function getMovieTime(showInformation){
  movieTime.value = dateFormatter.get24HourTime(showInformation.start_time)
}
function getCinemaName(showInformation){
  cinemaName.value = showInformation.cinema_name
}
onBeforeMount(async()=>{
  ...
  const showingInformation = await showingStore.fetchShowing(showing_id)
  if(!showingInformation){
    alert("cannot get showing information")
  }
  getMovieName(showingInformation)
  getMovieDate(showingInformation)
  getMovieTime(showingInformation)
  getCinemaName(showingInformation)
})
</script>
